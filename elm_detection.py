...
class elm_detection:
    ...
    def __init__(self, t, y):
        ...
        self.m = 500 # number of time windows used to calculate threshold curve
        self.h = 8 # half-width of gaussian smoothing window
        self.dt = self.t[len(self.t)//2] - self.t[(len(self.t)//2)-1] # time resolution
    ...
    def run(self):
        ...
        self.elm_sep = np.diff(self.elm_times) # get separation time between adjacent elms
    ...
    def get_delta(self):
        ...
        if (self.mode == 'fractional'):
            self.delta[self.d:] = (self.y[self.d:]-self.y[:-self.d])/self.y[:-self.d]
        else:
            self.delta[self.d:] = self.y[self.d:]-self.y[:-self.d]

        self.delta[np.where(self.delta<0)] = 0 # sets all negative derivatives to zero
